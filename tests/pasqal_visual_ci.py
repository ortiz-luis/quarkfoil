...
def main() -> int:
    ...
    try:
        ...
        # Selenium sizes Firefox's outer window. Compensate once for browser chrome
        # so the actual screenshot viewport is exactly the Beamer reference size.
        inner_w, inner_h = driver.execute_script("return [window.innerWidth, window.innerHeight]")
        driver.set_window_size(TARGET_WIDTH + (TARGET_WIDTH - inner_w), TARGET_HEIGHT + (TARGET_HEIGHT - inner_h))
        WebDriverWait(driver, 10).until(
            lambda d: d.execute_script("return window.innerWidth") == TARGET_WIDTH
            and d.execute_script("return window.innerHeight") == TARGET_HEIGHT
        )
        ...
        driver.execute_script(
            ...
            "const r=document.querySelector('.reveal');const ss=document.querySelector('.slides');"
        ...
        for index in range(count):
            driver.execute_script(
                ...
                "sections.forEach((s,i)=>{s.hidden=false;s.classList.remove('present','past','future');"
        ...
        cropped = [item["id"] for item in diagnostics if round(item["slide"]["w"]) != TARGET_WIDTH or round(item["slide"]["h"]) != TARGET_HEIGHT or abs(item["slide"]["x"]) > 0.5 or abs(item["slide"]["y"]) > 0.5]
        if broken:
